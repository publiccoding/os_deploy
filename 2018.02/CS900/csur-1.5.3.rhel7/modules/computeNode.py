    ...
    def computeNodeInitialize(self, computeNodeResources, versionInformationLogOnly, updateOSHarddrives):
        ...
        if result.returncode != 0:
            ...
            if '16s' in systemModel:
                logger.info('The system was determined to be a Superdome2 compute node. Checking its CPU model.')
                ...
                result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                out, err = result.communicate()
                logger.info('The output of the command (' + command + ") used to get the system's CPU model was: " + out.strip())
                if result.returncode != 0:
                    logger.error('There was a problem getting the cpu information.\n' + err + '\n' + out)
                    resultDict['errorMessages'].append('Unable to get the cpu information.')
                ...
                for line in cpudata:
                    if re.match('\\s*model\\s+:\\s+[2-9]{2}', line) != None:
                        try:
                            ...
                            resultDict['errorMessages'].append('There was a CPU model match error.')
                ...
                logger.info("The system's generation was determined to be: " + systemModel + '.')
            ...
            if updateOSHarddrives:
                if len(componentUpdateDict['Firmware']) != 0 and len(resultDict['hardDrivesMissingFirmware']) == 0:
