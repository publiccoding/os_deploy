...
class ComputeNodeInventory():
    ...
    def __init__(self, computeNodeDict, noPMCFirmwareUpdateModels, computeNodeResources, **kwargs):
        ...
        self.componentHeader = 'Component'
        self.componentUnderLine = '---------'
        self.csurVersionHeader = 'CSUR Version'
        self.csurVersionUnderLine = '------------'
        self.currentVersionHeader = 'Current Version'
        self.currentVersionUnderLine = '---------------'
        self.statusHeader = 'Status'
        self.statusUnderLine = '------'
        self.emptyDriverInventory = True
        self.emptySpecificDriverInventory = True
        self.printReportHeaders = True
    ...
    def __getFirmwareDict(self):
        ...
        for data in self.computeNodeResources:
            data = data.replace(' ', '')
            if 'Firmware' not in data and not firmwareFound:
                continue
            elif 'Firmware' in data:
                firmwareFound = True
                continue
            elif self.systemModel not in data and not started:
                continue
            elif self.systemModel in data and not started:
                started = True
                continue
            elif not started:
                continue
            elif re.match('\\s*$', data) and started:
                break
            else:
    ...
    def getComponentUpdateInventory(self, csurResourceDict):
        ...
        self.versionInformationLogger.info('{0:40} {1:25} {2:25} {3}'.format(self.componentHeader, self.csurVersionHeader, self.currentVersionHeader, self.statusHeader))
        self.versionInformationLogger.info('{0:40} {1:25} {2:25} {3}'.format(self.componentUnderLine, self.csurVersionUnderLine, self.currentVersionUnderLine, self.statusUnderLine))
        if '16s' not in self.systemModel:
        ...
        if '16s' not in self.systemModel:
            ...
            if self.emptyDriverInventory and self.emptySpecificDriverInventory:
                self.versionInformationLogger.info('Using inbox drivers.')
    ...
    def __getDriverInventory(self, hbaDrivers, localStorageDrivers):
        ...
        for data in self.computeNodeResources:
            ...
            if 'Drivers' not in data and not driversFound:
                ...
                driversFound = True
                continue
            elif driversFound and 'Software' in data:
                break
                ...
                if not self.hbaPresent and csurDriver in hbaDriverDict:
                    self.logger.info('Ignore HBA drivers since an HBA card is not present.')
                    continue
                if not self.localStoragePresent and csurDriver in localStorageDriverDict:
                    self.logger.info('Ignore local storage drivers since local storage is not present.')
                    continue
                ...
                if self.printReportHeaders:
                    self.versionInformationLogger.info('{0:40} {1:25} {2:25} {3}'.format(self.componentHeader, self.csurVersionHeader, self.currentVersionHeader, self.statusHeader))
                    self.versionInformationLogger.info('{0:40} {1:25} {2:25} {3}'.format(self.componentUnderLine, self.csurVersionUnderLine, self.currentVersionUnderLine, self.statusUnderLine))
                if installedDriverVersion != csurDriverVersion and csurDriverList[2] != 'None':
                    self.printReportHeaders = False
                    self.emptyDriverInventory = False
        ...
        for data in self.computeNodeResources:
            ...
            if 'Software' not in data and not softwareFound:
                ...
                if self.systemModel != 'DL380pGen8':
                    if self.systemModel != 'DL360pGen8':
                        if self.systemModel != 'DL360Gen9' and 'serviceguard' in csurSoftware:
                            continue
                        ...
                        installedSoftwareEpoch != csurSoftwareEpoch and csurSoftwareList[3] != 'None' and csurSoftware in sgNFSSoftwareDict and self.componentUpdateDict['sgNFSSoftware'].append(csurSoftwareList[3])
                    elif csurSoftware in sgSoftwareDict:
                        self.componentUpdateDict['sgSoftware'].append(csurSoftwareList[3])
                    else:
                        self.componentUpdateDict['Software'].append(csurSoftwareList[3])
            ...
            if not re.match('FusionIODriver', data) and not started:
                ...
                if result.returncode != 0:
                    ...
                    if installedDriverVersion != csurDriverVersion:
                        self.printReportHeaders = False
                        self.emptySpecificDriverInventory = False
