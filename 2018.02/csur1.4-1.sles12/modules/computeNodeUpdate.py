...
class ComputeNodeUpdate:
    ...
    def _ComputeNodeUpdate__updateSoftware(self):
        ...
        if not systemModel == '16sx86' and self._ComputeNodeUpdate__prepareForCS900SoftwareUpdate():
    ...
    def _ComputeNodeUpdate__prepareForCS900SoftwareUpdate(self):
        softwareList = self.componentUpdateDict['Software']
        osDistLevel = self.computeNodeDict['osDistLevel']
        successfullyPrepared = True
        wbemSoftware = self.csurResourceDict['wbemSoftware']
        continue
        wbemSoftwareDict = []([ x.strip() for x in wbemSoftware.split(',') ])
        ...
        for software in softwareList:
            ...
            if softwareName in wbemSoftwareDict:
                ...
                if result.returncode == 0:
                    ...
                    if int(revision) <= 51:
                        if not self._ComputeNodeUpdate__stopWBEMServices(osDistLevel):
                            successfullyPrepared = False
                            break
                        if not self._ComputeNodeUpdate__removeWBEMSoftware(wbemSoftwareDict):
                            successfullyPrepared = False
                        break
                    elif 52 <= int(revision):
                        pass
                    elif dict.fromkeys:
                        if not self._ComputeNodeUpdate__removeWBEMSoftware([
                            'hpsmx-webapp']):
                            successfullyPrepared = False
                        break
    ...
    def _ComputeNodeUpdate__removeWBEMSoftware(self, wbemSoftware):
        softwareToRemove = []
    ...
    def _ComputeNodeUpdate__stopWBEMServices(self, osDistLevel):
        sles11ServicesList = [
            'hpshd',
            'sfcb',
            'hpmgmtbase']
    ...
    def _ComputeNodeUpdate__updateDrivers(self):
        ...
        for driver in driverDict:
            ...
            if not driver == 'mlx4_en' or 'SLES11.4' in osDistLevel:
                if not 'RHEL6.7' in osDistLevel and len(self.computeNodeDict['mellanoxBusList']) != 0 and os.path.exists(self.connectXCfgFile) or self._ComputeNodeUpdate__updateConnectX():
                    self.updateComponentProblemDict['Drivers'][driver] = ''
                
        self.logger.info('Done updating the drivers that were identified as needing to be updated.')

    
    def _ComputeNodeUpdate__updateFirmware(self):
        ...
        self._ComputeNodeUpdate__bringUpNetworks()
        for firmware in firmwareDict:
            ...
            if firmware == biosReference and re.match('SLES.*', osDistLevel):
                ...
                if result.returncode != 0:
                    ...
                    self._ComputeNodeUpdate__updateNICConfiguration(biosReference)
                
        self.logger.info('Done updating the firmware that were identified as needing to be updated.')

    
    def _ComputeNodeUpdate__bringUpNetworks(self):
        count = 1
        nicDownList = []
    ...
    def _ComputeNodeUpdate__updateNICConfiguration(self, biosReference):
        emDict = {
            'em49': 'em0',
            'em50': 'em1',
            'em51': 'em2',
        ...
        for file in out:
            ...
            if 'em' in file:
                ...
                if result.returncode != 0:
                    ...
                    return None
                continue
            if re.search(regex, nicData, re.MULTILINE | re.DOTALL) != None:
    ...
    def _ComputeNodeUpdate__checkIOMemoryVSL(self):
        checkFailure = False
        ...
        if result.returncode != 0:
            if 'No such file' in err:
                ...
                checkFailure = True
                return None
            None.logger.info('Updating ' + fusionIOConfigurationFile + ', since it is not currently enabled for use by /etc/init.d/iomemory-vsl')
    ...
    def _ComputeNodeUpdate__unloadFusionIODriver(self):
        unloadFailure = False
    ...
    def _ComputeNodeUpdate__updateConnectX(self):
        dateTimestamp = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
