# Embedded file name: ./modules/cursesThread.py
        ...
        try:
            ...
            while not self.stop.isSet():
                ...
                try:
                    ...
                    scrollBarWindow = mainWindow.subpad(y, 3, 0, 0)
                    scrollBarWindow.border('|', '|', '-', '-', '+', '+', '+', '+')
                    conversationWindow = mainWindow.subpad(y, 0, 0, 2)
                        ...
                        for i in range(0, maxRowsConversationWindow):
                            ...
                            if i == len(wrappedMessagesList) - 1:
                                if y >= len(wrappedMessagesList):
                                    for i in range(0, y - len(wrappedMessagesList) - 2):
                                        scrollBarWindow.addstr(yPosition, 1, ' ', scrollBarBackground)
                                        yPosition += 1

                                break

                    scrollBarWindow.refresh(0, 0, 0, 0, y, 3)
                    conversationWindow.refresh(0, 0, 0, 2, y, x)
                except curses.error:
