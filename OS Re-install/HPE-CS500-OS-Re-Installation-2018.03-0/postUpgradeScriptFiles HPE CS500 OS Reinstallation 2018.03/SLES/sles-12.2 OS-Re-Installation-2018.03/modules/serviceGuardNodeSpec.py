...
def configureServiceguardQS(programParentDir, osDistLevel, cursesThread):
    errorMessage = ''
    logger = logging.getLogger('coeOSUpgradeLogger')
    logger.info('Installing Serviceguard QS and updating /etc/inittab.')
    cursesThread.insertMessage(['informative', 'Installing Serviceguard QS and updating /etc/inittab.'])
    cursesThread.insertMessage(['informative', ''])
    try:
        if osDistLevel == '11.4':
            qsRPM = glob.glob(programParentDir + '/qsRPMDir/serviceguard-qs-*sles11.x86_64.rpm')[0]
        else:
            qsRPM = glob.glob(programParentDir + '/qsRPMDir/serviceguard-qs-*sles12.x86_64.rpm')[0]
    except IndexError as err:
        logger.error('Failed to retrieve the Serviceguard QS RPM.\n' + str(err))
        errorMessage = 'Failed to retrieve the Serviceguard QS RPM; thus the Serviceguard QS will have to be configured manually.'
        return errorMessage

    command = 'rpm -Uvh ' + qsRPM
    result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    out, err = result.communicate()
    if result.returncode != 0:
        logger.error('Failed to install the Serviceguard QS RPM (' + qsRPM + ').\n' + err + '\n' + out)
        errorMessage = 'Failed to install the Serviceguard QS RPM; thus the Serviceguard QS will have to be configured manually.'
    if errorMessage == '':
        command = 'echo -e "#Quorum Server:\nqs:345:respawn:/opt/qs/bin/qs >> /var/log/qs/qs.log 2>&1" >> /etc/inittab'
        result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        out, err = result.communicate()
        if result.returncode != 0:
            logger.error('Failed to update /etc/inittab for the Serviceguard QS.\n' + err + '\n' + out)
            logger.info('The command used to update the inittab file was:\n' + command)
            errorMessage = 'Failed to update /etc/inittab for the Serviceguard QS; thus the inittab file will have to be updated manually.'
    logger.info('Done installing Serviceguard QS and updating /etc/inittab.')
    return errorMessage


def checkForQSRPMPresence(upgradeWorkingDir, osDist, osUpgradeVersion):
    logger = logging.getLogger('coeOSUpgradeLogger')
    while True:
        response = '_'
        if osDist == 'SLES':
            if osUpgradeVersion == '11.4':
                qsRPMList = glob.glob('/tmp/coeQSUpgrade/serviceguard-qs-*sles11.x86_64.rpm')
            else:
                qsRPMList = glob.glob('/tmp/coeQSUpgrade/serviceguard-qs-*sles12.x86_64.rpm')
        logger.info('The Serviceguard Quorum server RPM list for ' + osDist + ' ' + osUpgradeVersion + ' is: ' + str(qsRPMList) + '.')
        if len(qsRPMList) != 1:
            if not os.path.isdir('/tmp/coeQSUpgrade'):
                try:
                    os.mkdir('/tmp/coeQSUpgrade')
                except OSError as err:
                    logger.error('Unable to create the Serviceguard quorum server RPM directory /tmp/coeQSUpgrade.\n' + str(err))
                    print RED + 'Unable to create the Serviceguard quorum server RPM directory; create the directory and place the Serviceguard quorum server RPM in it; exiting program execution.' + RESETCOLORS
                    exit(1)

            while not response == '':
                response = raw_input(RED + "The Serviceguard quorum server RPM is missing from the /tmp/coeQSUpgrade directory. Place the RPM (serviceguard-qs-*.x86_64.rpm) in the directory and press Enter to continue or 'q' to quit." + RESETCOLORS)
                response = response.lower().strip()
                if response == 'q':
                    exit(1)
                elif response == '':
                    ...
                    logger.error('An invalid response (' + response + ') was provided.')
                    print RED + 'An invalid response was provided; please try again.' + RESETCOLORS
                    continue

        else:
            try:
                qsRPMDir = upgradeWorkingDir + '/qsRPMDir'
                os.mkdir(qsRPMDir)
                shutil.copy2(qsRPMList[0], qsRPMDir)
                break
            except OSError as err:
                logger.error("Unable to copy the Serviceguard quorum server RPM to '" + qsRPMDir + "'.\n" + str(err))
                print RED + "Unable to copy the Serviceguard quorum server RPM to '" + qsRPMDir + "'; fix the problem and try again; exiting program execution." + RESETCOLORS
                exit(1)