...
def main():
    ...
    if options.p:
        if not serverModel == 'ProLiant DL380p Gen8':
            print serverModel == 'ProLiant DL360p Gen8' or RED + "The '-p' option is only for Serviceguard NFS servers; exiting program execution." + RESETCOLORS
            exit(1)
        updateClusterCfg()
    else:
        try:
                    ...
                    if serverModel == 'ProLiant DL320e Gen8 v2' or serverModel == 'ProLiant DL360 Gen9':
                        ...
                        if serverModel == 'ProLiant DL320e Gen8 v2' or serverModel == 'ProLiant DL360 Gen9':
                            if 'configureServiceguardQS' not in progressDict:
                                ...
                                len(errorMessage) > 0 and errorMessageList.append(errorMessage)
                        else:
