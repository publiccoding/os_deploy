...
class ComputeNodeInventory:
	...
	def __init__(self, computeNodeDict, noPMCFirmwareUpdateModels, computeNodeResources):
		...
		self.versionInformationLogger.info('{0:40}'.format('Version information for Compute Node ' + re.sub('Logger', '', loggerName) + ':') + '\n')
	...
	def __getStorageFirmwareInventory(self):
		...
		command = arrayCfgUtilFile + " ctrl all show status"
	...
	def __getNICFirmwareInventory(self):
		nicCardModels = [] #This contains the list of NIC cards that were found, e.g. ['HP331FLR', 'HP560SFP'].
		count = 0
		...
		#Get a unique list of NIC card models from the bus list, which has contents such as: ['03:00.0 HP331FLR', '44:00.0 HP560SFP', '47:00.0 HP560SFP'].
		...
		nicCardList = re.findall('^[e,m,p,t,h,0-9]{3,}', out, re.MULTILINE|re.DOTALL)
	...
	def __getNicBusList(self):
		nicBusList = []
		busDict = {}
		modelCrossRefDict = {'NetXtreme BCM5719' : 'HP331FLR', 'Intel Corporation 82599' : 'HP560SFP', 'Mellanox Technologies' : 'HP544QSFP'}
		...
		for device in deviceList:
			if 'Ethernet controller' in device or 'Network controller' in device:
				if self.systemModel == 'DL580G7' or self.systemModel =='DL980G7' or self.systemModel =='BL680cG7': # NC375T, NC375i, NC550SFP
					bus = re.match('([0-9,a-f]{2}:[0-9,a-f]{2}\.[0-9]).*(NC[0-9]+[a-z]{1,3}).*', device, re.IGNORECASE|re.MULTILINE|re.DOTALL)
				elif self.systemModel == 'DL380pGen8':
					bus = re.match('([0-9,a-f]{2}:[0-9,a-f]{2}\.[0-9]).*(NetXtreme BCM5719|NC552SFP).*', device, re.MULTILINE|re.DOTALL)
				elif self.systemModel == 'DL580Gen8' or self.systemModel == 'DL580Gen9' or self.systemModel == 'DL320eGen8v2':
					bus = re.match('([0-9,a-f]{2}:[0-9,a-f]{2}\.[0-9]).*Product Name:.*(331FLR|331T|544\+QSFP|560SFP\+|331T|561T|332i|332T)\s+Adapter.*', device, re.MULTILINE|re.DOTALL)
	...
	'''
	This function is used to check if storage is local.
	'''
		...
		for line in fioStatusList:
			...
			if "Firmware" in line or re.search('PCI:[0-9a-f]{2}:[0-9]{2}\.[0-9]{1}', line):
				if "Firmware" in line:
					...
					ioDIMMStatusDict['bus'] = re.match('.*([0-9,a-f]{2}:[0-9]{2}\.[0-9]{1})', line).group(1)
