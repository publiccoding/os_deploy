...
class ComputeNodeUpdate:
	...
	def __updateDrivers(self):
		...
		for driver in driverDict:
			...
			if driver == 'iomemory_vsl':
				#Before backing up /etc/sysconfig/iomemory-vsl make sure it can be controlled by /etc/init.d/iomemory-vsl
				if self.__checkIOMemoryVSL():
					self.updateComponentProblemDict['Drivers'][driver] = ''
					continue

				#Before performing the driver update unload the previous driver.
			...
			if result.returncode != 0:
				...
				#Also, make an attempt to check if there were issues that did not cause a result code failure, e.g. Warning: weak-updates symlinks might not be created.
				#The user will have to check the log file to determine if the warning can be ignored.
				if re.search('warning', out, re.MULTILINE|re.DOTALL|re.IGNORECASE) != None: 
	...
	def __bringUpNetworks(self):
		...
		#Get the list of NIC interfaces that are down.
	...
	'''
	This function is used to make sure that the resource ENABLED is set to 1 (ENABLED=1).
	...
	def __checkIOMemoryVSL(self):
		...
		if result.returncode != 0:
			...
			#Check to make sure that the resource is even present; which is should be."
			...
			self.logger.debug("The output of the command " + cmd + " used to check ENABLED= in " + fusionIOConfigurationFile + " was:\n" + enabledCurrent)
			...
			self.logger.debug("The command used to update " + fusionIOConfigurationFile + " was: " + cmd + ".")
	...
	#End __checkIOMemoryVSL(self):


	'''
	This function is used to make sure the FusionIO driver (iomemory-vsl) is unloaded
	before a FusionIO driver or firmware update is performed.
	'''
	...
	#End __unloadFusionIODriver(self):
