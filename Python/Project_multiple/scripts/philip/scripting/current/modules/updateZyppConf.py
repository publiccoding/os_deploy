...
def updateZyppConf(logFile):
	...
	#This is the script that will be written out and then ran by calling it with subprocess.
	script = '''
		...
		if [[ ! `egrep "^\s*multiversion\s*=\s*" ${zyppConf}` ]]; then
			echo "Using the command \"sed -i '0,/^\s*#\+\s*multiversion\s*=\s*.*$/s//multiversion = provides:multiversion(kernel)/' $zyppConf\" to update zypp.conf"
			sed -i '0,/^\s*#\+\s*multiversion\s*=\s*.*$/s//multiversion = provides:multiversion(kernel)/' $zyppConf
		else
			echo "Using the command \"sed -i '0,/^\s*multiversion\s*=\s*.*$/s//multiversion = provides:multiversion(kernel)/' $zyppConf\" to update zypp.conf"
		...
		if [[ ! `egrep "^\s*multiversion.kernels\s*=\s*" ${zyppConf}` ]]; then
			echo "Using the command \"sed -i '0,/^\s*#\+\s*multiversion.kernels\s*=\s*.*$/s//multiversion.kernels = latest,latest-1,running/' $zyppConf\" to update zypp.conf"
			sed -i '0,/^\s*#\+\s*multiversion.kernels\s*=\s*.*$/s//multiversion.kernels = latest,latest-1,running/' $zyppConf
		else
			echo "Using the command \"sed -i '0,/^\s*multiversion.kernels\s*=\s*.*$/s//multiversion.kernels = latest,latest-1,running/' $zyppConf\" to update zypp.conf"
	...
	'''	
        ...
        try:
		#This is the specific log file for the script.
                fh = open(logFile, 'w')
