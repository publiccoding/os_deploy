# Embedded file name: /hp/support/health/bin/modules/collectComponentInformation.py
import re
import getpass
import logging
import time
...
class GetComponentInformation:
    ...
    def getComponentInformation(self, versionInformationLogOnly, updateOSHarddrives):
        ...
        self.healthResourceDict['multipleUpgradesRequiredDict'] = {'SAN Switches': [],
         '3PAR': []}
        ...
        if 'Scale-up' in systemTypeDict:
            ...
            componentResourceList = self.__getComponentResources(['Compute Node'], healthBasePath)
            ...
            componentResourceList = self.__getComponentResources(componentList, healthBasePath)
            if componentResourceList[0]:
                self.__exitOnError("An error occurred while collecting component resources; check the application's log file for errors; exiting program execution.")
                                    ...
                                    if 'invalid password' in errorMessage:
                                        if count == 0:
                                            if self.__confirmInput(component, ip, passwordFailure='passwordFailure'):
                                                ...
                                                break
                                            count += 1
                                        elif self.__tryAgainQuery(component, resultDict['errorMessages']):
                                            username, password = self.__getCredentials(component, ip)
                                        else:
        ...
        if any((len(self.healthResourceDict['multipleUpgradesRequiredDict'][i]) != 0 for i in self.healthResourceDict['multipleUpgradesRequiredDict'])):
            try:
                ...
                for i in self.healthResourceDict['multipleUpgradesRequiredDict']:
                    if len(self.healthResourceDict['multipleUpgradesRequiredDict'][i]) != 0:
                        if iteration == 0:
                            iteration += 1
                        else:
    ...
    def __getComponentResources(self, componentList, healthBasePath):
