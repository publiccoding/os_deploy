# Embedded file name: /hp/support/health/bin/modules/pexpect/__init__.py
...
def run(command, timeout = -1, withexitstatus = False, events = None, extra_args = None, logfile = None, cwd = None, env = None):
    ...
    **Examples**
    ...
    """
    return _run(command, timeout=timeout, withexitstatus=withexitstatus, events=events, extra_args=extra_args, logfile=logfile, cwd=cwd, env=env, _spawn=spawn)


def runu(command, timeout = -1, withexitstatus = False, events = None, extra_args = None, logfile = None, cwd = None, env = None, **kwargs):
    ...
    """
    return _run(command, timeout=timeout, withexitstatus=withexitstatus, events=events, extra_args=extra_args, logfile=logfile, cwd=cwd, env=env, _spawn=spawnu, **kwargs)


def _run(command, timeout, withexitstatus, events, extra_args, logfile, cwd, env, _spawn, **kwargs):
    if timeout == -1:
        child = _spawn(command, maxread=2000, logfile=logfile, cwd=cwd, env=env, **kwargs)
    else:
        child = _spawn(command, timeout=timeout, maxread=2000, logfile=logfile, cwd=cwd, env=env, **kwargs)
...
class spawn(object):
    ...
    if PY3:
        ...
        allowed_string_types = (basestring,)
    ...
    def __init__(self, command, args = [], timeout = 30, maxread = 2000, searchwindowsize = None, logfile = None, cwd = None, env = None, ignore_sighup = True, echo = True):
        ...
        if not _platform.startswith('solaris'):
            self.use_native_pty_fork = not _platform.startswith('sunos')
            try:
            ...
            self.command = command is None and None
            self.args = None
    ...
    def _spawn(self, command, args = []):
        ...
        raise self.pid is None or AssertionError, 'The pid member must be None.'
        if not self.command is not None:
            raise AssertionError, 'The command member must not be None.'
            ...
            if self.pid == pty.CHILD:
                ...
                if self.cwd is not None:
                    os.chdir(self.cwd)
                self.env is None and os.execv(self.command, self.args)
            else:
