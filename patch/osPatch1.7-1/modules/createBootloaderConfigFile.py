        ...
        with open('/boot/grub/menu.lst') as f:
            for line in f:
                ...
                if 'gfxmenu' in line:
                    ...
                    kernel = re.sub(' +', ' ', line)
                    failsafeKernel = re.sub('resume=(/[a-z,0-9,-,_]*)+\\s+', failsafeResources + ' ', kernel)
...
def configureEliloBootLoader(kernelList, loggerName):
    ...
    for kernelVersion in finalKernelList:
        kernelVersion = kernelVersion[:-2]
        vmlinuzList.append('vmlinuz-' + kernelVersion + kernelSuffix)
        initrdList.append('initrd-' + kernelVersion + kernelSuffix)
    ...
    for line in eliloConfData:
        ...
        if 'image' in line and re.search(currentKernelPattern, line):
            ...
            initrd = re.sub(' +', ' ', line)
        elif 'root' in line and 'append' not in line:
            root = re.sub(' +', ' ', line)
        elif 'append' in line and 'noresume' not in line:
            append = re.sub(' +', ' ', line)
            append = re.sub('" ', '"', append)
            append = re.sub(' "$', '"', append)
            failsafeAppend = re.sub('resume=(/[a-z,0-9,-,_]*)+\\s+', failsafeResources + ' ', append)
