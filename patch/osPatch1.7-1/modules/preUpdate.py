...
def setPatchDirectories(patchResourceDict, loggerName):
    ...
    if options.a or options.k:
        ...
        if result.returncode != 0:
            ...
            exit(1)
        if 'Superdome' in out and 'SP3' in patchResourceDict['osDistLevel']:
    ...
    if options.a:
        if os.path.exists(kernelDir):
            os.path.exists(osDir) or logger.error('Option -a was selected, however, both of the OS and kernel patch directories (' + kernelDir + ', ' + osDir + ') are not present.')
...
def updateBootloaderCFG(loggerName):
    ...
    result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    out, err = result.communicate()
    logger.debug('The output of the command (' + command + ') used to get the parameters passed to the kernel during startup was: ' + out.strip())
    if result.returncode != 0:
