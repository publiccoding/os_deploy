...
def setPatchDirectories(patchResourceDict, loggerName):
    ...
    if options.a:
        if not (os.path.exists(kernelDir) and os.path.exists(osDir) and os.path.exists(additionalRPMSDir)):
            logger.error('Option -a was selected, however, all of the RPM directories (' + kernelDir + ', ' + osDir + ', ' + additionalRPMSDir + ') are not present.')
        ...
        if not os.path.exists(kernelDir):
            ...
            patchDirList.append(kernelDir)
    elif not (os.path.exists(osDir) and os.path.exists(additionalRPMSDir)):
        logger.error('Option -o was selected, however, both the OS and additional RPM directory (' + osDir + ', ' + additionalRPMSDir + ') are not present.')
