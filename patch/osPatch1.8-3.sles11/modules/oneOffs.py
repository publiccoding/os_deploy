...
class OneOffs:
    ...
    def removeRPMs(self, patchResourceDict):
        ...
        if len(rpmList) != 0:
            ...
            self.logger.debug('The output of the command (' + command + ') used to remove the pre-identified RPMs for removal was: ' + out.strip())
    ...
    def addRPMs(self, patchResourceDict):
        ...
        try:
            patchBaseDir = patchResourceDict['patchBaseDir'].rstrip('/')
            additionalSubDir = patchResourceDict['additionalSubDir']
            additionalDir = patchBaseDir + '/' + additionalSubDir + '/'
            rpmReferenceList = re.sub('\\s+', '', patchResourceDict['rpmsToAdd']).split(',')
            rpmReferenceDict = dict([ x.split(':') for x in rpmReferenceList ])
            self.logger.info('The RPM reference dictionary was determined to be: ' + str(rpmReferenceDict))
        except KeyError as err:
            self.logger.error('The resource key (' + str(err) + ') was not present in the resource file.')
            return False

        rpmNameList, result = self.__checkRPMsForAddition(rpmReferenceDict.keys())
        if not result:
            self.logger.error('Problems were encountered while getting the updated list of RPMs to add.')
            return False
        if len(rpmNameList) != 0:
            rpmList = [ rpmReferenceDict[x] for x in rpmNameList ]
            for rpm in rpmList:
                if count < len(rpmList):
                    rpmsForAddition += additionalDir + rpm + ' '
                else:
                    rpmsForAddition += additionalDir + rpm
                count += 1

            command = 'rpm -ivh ' + rpmsForAddition
            result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            out, err = result.communicate()
            self.logger.debug('The output of the command (' + command + ') used to add the pre-identified RPMs for addition was: ' + out.strip())
            if result.returncode == 0:
                self.logger.info('Successfully added the following RPMs: ' + rpmsForAddition)
            else:
    ...
    def __checkRPMsForAddition(self, rpmList):
        ...
        for rpm in rpmList:
            ...
            result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            out, err = result.communicate()
            self.logger.debug('The output of the command (' + command + ') used to check if an RPM was installed was: ' + out.strip())
            if result.returncode == 0:
                continue
    ...
    def __checkRPMsForRemoval(self, rpmsToRemoveList):
        ...
        result = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        out, err = result.communicate()
        self.logger.debug('The output of the command (' + command + ') used to get a list of the installed RPMs was: ' + out.strip())
        if result.returncode != 0:
